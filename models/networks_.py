...
class GlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        model_down = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
        ### downsample
    ...
    def forward(self, input):
        ...
        # print(input.data.shape); # torch.Size([1, 3, 256, 256])
        out_down = self.model_down(input); # print(out_down.data.shape) # torch.Size([1, 1024, 16, 16])
